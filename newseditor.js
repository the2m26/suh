...
let _allNewsEditorPosts = [];

async function renderNewsEditorPage() {
  const el = document.getElementById('newseditor-list');
  if (!el) return;
  const addBtn = document.querySelector('#page-newseditor button[onclick="openNewsEditorForm(null)"]');
  if (addBtn) addBtn.style.display = canAdd('newseditor') ? '' : 'none';
  ...
  _allNewsEditorPosts = data || [];
  _renderNewsEditorFilterBar();
  _applyNewsEditorTopicFilter();
}

// ⚠️ 2026-07-24 нэмэв: "Мэдээний агрегат" гэсэн жижиг дэд гарчгийн оронд,
// "Мэдээ, мэдээлэл" хуудастай ЯГ ижил topic-ын шүүлтүүр dropdown байрлуулав —
// мэдээ бичигч ч мөн адил ангиллаар шүүж, засах/устгах мэдээгээ хялбар олно.
function _renderNewsEditorFilterBar() {
  const wrap = document.getElementById('newseditor-filter-bar');
  if (!wrap || wrap.dataset.built) return;
  wrap.dataset.built = '1';
  wrap.innerHTML = `<select id="newseditor-topic-filter" onchange="_applyNewsEditorTopicFilter()" style="width:auto;min-width:200px">
    <option value="">Бүгдийг харах</option>
    ${NEWS_TOPICS.map(t => `<option value="${t.value}">${esc(t.label)}</option>`).join('')}
  </select>`;
}

function _applyNewsEditorTopicFilter() {
  const el = document.getElementById('newseditor-list');
  if (!el) return;
  const filterVal = document.getElementById('newseditor-topic-filter')?.value || '';
  const data = filterVal ? _allNewsEditorPosts.filter(p => p.topic === filterVal) : _allNewsEditorPosts;
  if (!data.length) { el.innerHTML = '<div class="empty-state">Одоогоор мэдээ үүсгээгүй байна</div>'; return; }
  const _canEditNews = canWrite('newseditor'), _canDelNews = canDelete('newseditor');
  ...
  // Огноо ба Гарчиг хооронд Ангилал (Topic) баганыг нэмэв.
  el.innerHTML = `<div style="overflow-x:auto">
  <table class="data-table" style="min-width:1000px">
    <thead>
      <tr style="background:rgba(11,26,46,0.8)">
        <th style="width:150px">ОГНОО</th>
        <th style="width:170px">АНГИЛАЛ</th>
    ...
    <tbody>${data.map(p => `
      <tr style="cursor:pointer" onclick="openNewsEditorForm(${p.id})">
        ...
        <td class="dt-text">${esc(_newsTopicLabel(p.topic)) || '—'}</td>
        ...
        <td onclick="event.stopPropagation()"><div class="flex gap-8">
          ${_canEditNews ? `<button class="btn btn-ghost btn-sm" onclick="openNewsEditorForm(${p.id})" style="padding:4px;display:inline-flex;align-items:center;color:var(--text-muted)" title="Засах"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 1 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>` : ''}
          ${_canDelNews ? `<button class="btn btn-ghost btn-sm" onclick="deleteNewsPost(${p.id})" style="padding:4px;display:inline-flex;align-items:center;color:var(--danger)" title="Устгах"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button>` : ''}
          ${!_canEditNews && !_canDelNews ? '<span style="color:var(--text-muted);font-size:11px">—</span>' : ''}
        </div></td>
      </tr>`).join('')}
    </tbody>
  </table>
...
// ⚠️ 2026-07-24 нэмэв: Ямар ч эх сурвалжаас (өөр вэбсайт, Word гэх мэт) copy+paste
// хийхэд, тэдгээрийн ӨӨРИЙН фонт хэмжээ/family/өнгө, параграф/мөр хоорондын зай,
// дэвсгэр өнгийг АВЧИРАХГүйгээр, зөвхөн ЭГЭЭ ТЕКСТ (параграф бүтцийг хадгалж)
// оруулж, платформын дефолт (.news-body CSS) стандартад автоматаар оруулна.
// Гараар бичихэд ч аюулгүй — зүгээр plain text л оролцоно.
function _setupNewsEditorPasteHandler() {
  const el = document.getElementById('newseditor-body');
  if (!el || el.dataset.pasteHandlerSet) return;
  el.dataset.pasteHandlerSet = '1';
  el.addEventListener('paste', function (e) {
    e.preventDefault();
    const text = (e.clipboardData || window.clipboardData).getData('text/plain');
    if (!text) return;
    // Мөр (шинэ мөр)-ийг тус тусад нь <p> болгож, HTML тусгай тэмдэгтийг esc-лээд
    // insertHTML-ээр оруулна — үүгээр эх сурвалжийн style/font бүгд шид алга болно.
...
function openNewsEditorForm(id) {
  if(!id && !canAdd('newseditor')) { toast('Танд шинэ мэдээ vvсгэх эрх байхгvй байна', 'error'); return; }
...
// ------------------------------------------------------------
// ⚠️ 2026-07-24 нэмэв: Supabase getPublicUrl()-ээс буцаж ирсэн бүтэн URL-аас
// "news-media" bucket доторх ЗАМ (path)-ыг задалж авна — устгах үед хэрэгтэй.
function _newsMediaPathFromUrl(url) {
  if (!url) return null;
  const marker = '/storage/v1/object/public/news-media/';
  const idx = url.indexOf(marker);
  return idx === -1 ? null : url.slice(idx + marker.length);
}
async function _deleteNewsMediaByUrl(url) {
  const path = _newsMediaPathFromUrl(url);
  if (!path) return;
...
// ⚠️ 2026-07-24 засав: Зураг устгахад Supabase Storage-с ч БОДИТООР устгаж,
// зайг hэмнэдэг болгов (өмнө нь зүгээр жагсаалтаас (массиваас) л hасдаг байсан,
// storage дотор нь мөнхөд үлддэг байсан).
async function _removeNewsPhoto(i) {
  const removed = _newsEditorPhotos[i];
  _newsEditorPhotos.splice(i, 1);
  _renderNewsEditorPhotoList();
  if (removed) await _deleteNewsMediaByUrl(removed.url);
}
...
async function onNewsPdfFileSelected(input) {
  ...
  // ⚠️ 2026-07-24 нэмэв: Шинэ PDF сонгоход, ХУУЧИН PDF байсан бол storage-с
  // устгаад л шинийг оруулна — сольж байгаа үед хуучин файл үүрд үлдэхгүй.
  const oldPath = _newsEditorPdfPath;
  ...
  _renderNewsEditorPdfStatus();
  input.value = '';
  if (oldPath) await _deleteNewsMediaByUrl(oldPath);
}
function _renderNewsEditorPdfStatus() {
  const el = document.getElementById('newseditor-pdf-status');
  if (!el) return;
  el.innerHTML = _newsEditorPdfPath
    ? `<a href="${esc(_newsEditorPdfPath)}" target="_blank" rel="noopener">📄 PDF харах</a> <button class="btn btn-ghost btn-sm" style="color:var(--danger)" onclick="_removeNewsEditorPdf()">✕</button>`
    : '<span style="font-size:12px;color:var(--text-muted)">PDF алга</span>';
}
async function _removeNewsEditorPdf() {
  const removed = _newsEditorPdfPath;
  _newsEditorPdfPath = null;
  _renderNewsEditorPdfStatus();
  if (removed) await _deleteNewsMediaByUrl(removed);
}

// ------------------------------------------------------------
// ХАДГАЛАХ (ноорог / паблиш)
// ------------------------------------------------------------
// 2026-07-26 нэмэв: contenteditable-ийн raw innerHTML-ийг шууд хадгалах нь
// stored XSS эрсдэлтэй (эдитор эрх алдагдвал/буруу paste хийвэл, тэр код
// БvХ vзэгч — Админ хvртэл — харах бvрдээ ажиллана). Хадгалахын өмнө
// sanitizeNewsBody()-оор цэвэрлэнэ (функц нь news.js-д тодорхойлогдсон,
// энд зөвхөн дуудна — давхар тодорхойлохгvй, файл эвдрэлээс сэргийлэх).
async function saveNewsPost(publish) {
  if(!(_newsEditorEditingId ? canWrite('newseditor') : canAdd('newseditor'))) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна', 'error'); return; }
  ...
  const body = sanitizeNewsBody(document.getElementById('newseditor-body').innerHTML.trim());
  ...
  if (publish) {
    // 2026-07-30 засав: Гараар баталгаажуулах шат үгүй, шууд АВТОМАТААР бүх
    // сууц өмчлөгч рүү push мэдэгдэл (дуут+баннер+PWA badge) илгээнэ.
    ...
    await triggerPushForRecipients(recipients, title);
    toast('Мэдээ болон push мэдэгдэл илгээгдлээ ✓', 'success');
  }
  closeNewsEditorForm();
}

async function deleteNewsPost(id) {
  if(!canDelete('newseditor')) { toast('Танд энэ vйлдлийг хийх эрх байхгvй байна', 'error'); return; }
  if (!confirm('Энэ мэдээг бүрмөсөн устгах уу?')) return;
  // ⚠️ 2026-07-24 нэмэв: Мэдээг бүрмөсөн устгахаас ӨМНӨ, түүнд хамаарах бүх
  // зураг/PDF-ийн замыг татаж авна — DB мөрийг устгасны дараа эдгээрийг
  // storage-с ч устгаж, зайг hэмнэнэ.
  ...
  if (post) {
    ...
    for (const p of photos) await _deleteNewsMediaByUrl(p.url);
    if (post.pdf_path) await _deleteNewsMediaByUrl(post.pdf_path);
  }
  toast('Устгагдлаа ✓', 'success');
  // 2026-07-27 засав: record_label-ыг өмнө нь NULL дамжуулдаг байсан тул
  // Протокол хvснэгэлд "Объект" багана хоосон (—) харагдаж, аль мэдээг
  // устгасныг таних боломжгvй байсан — одоо мэдээний гарчгийг дамжуулна.
  logActivity('delete', 'news', id, post?.title || null);
  renderNewsEditorPage();
}
