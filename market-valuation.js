...
function mvShowTooltip(evt, text) {
  ...
  // ⚠️ Дэлгэцний баруун/доод ирмэгээс давж гарахаас сэргийлнэ — эхлээд хулганы
  // баруун талд байрлуулж үзээд, хэрэв тэр байрлалд tooltip дэлгэц дундаас
  // давж гарах бол зүүн тал/дээш рүү нь эргүүлнэ.
  const margin = 8;
  const rect = tip.getBoundingClientRect();
  let left = evt.clientX + 12;
  if (left + rect.width + margin > window.innerWidth) left = evt.clientX - rect.width - 12;
  if (left < margin) left = margin;
  let top = evt.clientY - 26;
  if (top < margin) top = evt.clientY + 16;
