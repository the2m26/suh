...
// employees (employees.js), daysUnpaidForResident/daysUnpaidForBusiness/
...
function openNotifyFor(kind, id, label) {
  if(!id) { toast('Мэдэгдэл илгээх хүн олдсонгүй — эхлээд бүртгэл үүсгэнэ үү', 'error'); return; }
  ...
  setTimeout(()=>{
    ...
    // 2026-07-27 нэмэв: Тоот/Сууц өмчлөгч/ААН-ий дэлгэрэнгүйгээс "Мэдэгдэл"
    // товч дарж ирэхэд, Төрлийг автоматаар "Нэхэмжлэл" болгож, Агуулгыг batch
    // нэхэмжлэлтэй ЯГ ИЖИЛ форматаар (buildInvoiceContentFor(), accounting-
    // bridge.js) автоматаар үүсгэж бэлдэнэ — давтан/тусад нь нэхэмжлэх
    // шаардлагатай үед гараар дахин бичихгүйгээр шууд илгээхэд бэлэн байна.
    // Илгээгч дараа нь Төрөл/Агуулгыг чөлөөтэй өөрөөр ч сольж болно.
    if ((kind === 'resident' || kind === 'business') && typeof buildInvoiceContentFor === 'function') {
      const entity = kind === 'resident' ? residents.find(r=>r.id===id) : businesses.find(b=>b.id===id);
      if (entity) {
        document.getElementById('notif-category').value = 'invoice';
        buildInvoiceContentFor(kind, entity).then(content => {
          document.getElementById('notif-content').value = content;
...
// аль хэдийн байгаа daysUnpaidForResident/Business, _bizThresholds
...
function resolveNotificationRecipients() {
  ...
  if(kind === 'resident') {
    if(filter === 'specific') {
      ...
      return r ? [{name:(r.firstname||'')+' '+(r.lastname||''), email:r.email||'', phone:r.phone||'', ref_type:'resident', ref_id:r.id, apt:r.apt}] : [];
    }
    return residents.filter(r=>{
      if(!r) return false;
      if(filter==='all') return true;
      const mu = daysUnpaidForResident(r);
      if(filter==='pending') return mu>=1 && mu<overdueThreshold;
  ...
  if(kind === 'business') {
    ...
    return businesses.filter(b=>{
      ...
      if(filter==='pending') return mu>=1 && mu<th.overdue;
...
// ⚠️ Дундын "суваг бүрээр илгээх" функц — In-app DB-д хадгалагдаад,
// userapp-react-ийн Inbox-оор бодитоор хүргэгдэнэ (push-той хамт). Мэйл/СМС
...
async function renderNotifications() {
  ...
  // 2026-07-30 нэмэв: хүлээн авагч бүрийн "Уншсан" тоог нэг дор татна.
  const ids = list.map(n => n.id);
  const { data: seenRows } = await sb.rpc('get_notification_seen_counts', { p_ids: ids });
  const seenMap = {};
  (seenRows || []).forEach(r => { seenMap[r.notification_id] = r; });

  el.innerHTML = `<table class="data-table">
    <thead><tr><th>Хугацаа</th><th>Торол</th><th>Хүлээн авагч</th><th>Илгээгч</th><th>Гарчиг</th><th>Агуулга</th><th>Тоо</th><th>Уншсан</th><th>Суваг</th></tr></thead>
    <tbody>${list.map(n => {
      const seen = seenMap[n.id];
      const seenStr = seen ? `${seen.seen_count}/${seen.total_recipients}` : '—';
      return `
      <tr class="clickable-row" style="cursor:pointer" onclick="openNotifDetail(${n.id})">
        ...
        <td class="dt-mono">${esc(seenStr)}</td>
...
// (2026-07-30: fintax.js-ээс энд зөв байрлалд нь шилжүүлэв — санхүү/татварын
// тайлангийн модультой хамааралгүй байсан тул)
// ============================================================
let _callLogAll = [];
  ...
  body.innerHTML = `<table class="data-table">
    ...
    <tbody>${list.map(r => {
      ...
      <tr data-id="${r.id}">
        ...
        <td><span class="tag">${isNew ? 'Шинэ' : 'Хянасан'}</span></td>
        <td>${canReply ? `<button class="btn btn-sm btn-primary" onclick="replyToFeedback(${r.id})">Хариулах</button>` : ''}</td>
      </tr>`;
    }).join('')}
    </tbody>
  </table>`;
}
// ⚠️ 2026-07-30 засав: "Шинэ"⇄"Хянасан" toggle товчны оронд "Хариулах"
// нэг товч болгов — дарахад (1) статусыг "Хянасан" болгож, (2) CC center-ийн
// "Илгээх" tab руу шилжиж, тухайн сууц өмчлөгчийг хүлээн авагчаар автоматаар
// сонгоод, гарчиг/агуулгыг эх зурвасыг дурдсан загвараар урьдчилан бөглөнө —
// ажилтан зөвхөн хариултаа бичээд шууд илгээхэд бэлэн болно.
async function replyToFeedback(id) {
  const row = _callLogAll.find(r => r.id === id);
  if (!row) return;
  if (row.status === 'new') {
    const { error } = await sb.from('feedback_requests').update({ status: 'reviewed' }).eq('id', id);
    if (!error) row.status = 'reviewed';
  }
  const resident = residents.find(r => String(r.apt) === String(row.apt));
  if (!resident) { toast('Энэ тоотод бүртгэлтэй сууц өмчлөгч олдсонгүй', 'error'); loadCallLogInbox(); updateCallLogBadge(); return; }
  const label = `${row.apt} — ${resident.firstname || ''} ${resident.lastname || ''}`.trim();

  showPage('communications');
  setTimeout(() => {
    switchNotifTab('send', document.querySelector('#notif-tabs .tab'));
    const kindSel = document.getElementById('notif-recipient-kind');
    kindSel.value = 'resident';
    onNotifRecipientKindChange();
    const filterSel = document.getElementById('notif-recipient-filter');
    filterSel.value = 'specific';
    onNotifRecipientFilterChange();
    selectNotifSpecific(resident.id, label);
    document.getElementById('notif-category').value = 'notice';
    document.getElementById('notif-title-input').value = 'Таны илгээсэн санал, хүсэлтэд хариу';
    document.getElementById('notif-content').value = `Таны илгээсэн:\n"${row.content || ''}"\n\nХариу:\n`;
    if (typeof updateNotifPreview === 'function') updateNotifPreview();
  }, 60);
...
// ============================================================
// ИРСЭН САНАЛ, ХҮСЭЛТ — sidebar badge (Төлбор төлөлт-ийн overdue-badge-тэй
// ЯГ ИЖИЛ загвар)
// ============================================================
