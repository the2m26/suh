...
async function sendNotification() {
  ...
  const kindLabel = kind==='resident'?'Сууц өмчлөгч':kind==='business'?'ААН':'СӨХ';
  // ⚠️ 2026-07-19: "Тухайлсан" төрлийн үед өмнө нь зөвхөн ерөнхий шүүлтүүрийн нэрийг
  // ("Төлбөр төлөгч (тухайлсан)") харуулдаг байсан — бодит хүний нэр огт харагддаггүй
  // байсныг Илгээгдсэн хүснэгэлд ажиглаж олов. Одоо тухайлсан үед бодит нэрийг харуулна.
  const isSpecific = filter === 'specific' || filter === 'specific_employee';
  const recipientLabel = isSpecific
    ? `${recipients[0]?.name || '—'} (${kindLabel})`
    : `${NOTIF_FILTER_LABELS[filter]} (${kindLabel})`;
...
// ============================================================
// ИЛГЭЭГДСЭН — түүхийн жагсаалт (2026-07-19: шvvлтvvр/хайлт/дэлгэрэнгvй нэмэв)
// ============================================================
function populateNotifLogYearOptions() {
  ...
  if(sel.dataset.yearsKey === key) return; // өөрчлөгдөөгүй бол дахин зурахгvй
  const curVal = sel.value;
  sel.innerHTML = '<option value="">Бvх он</option>' + years.map(y=>`<option value="${y}">${y}</option>`).join('');
  sel.value = curVal;
...
function populateNotifLogDayOptions() {
  ...
  el.innerHTML = '<option value="">Бvх өдөр</option>' + opts.join('');
  el.dataset.init = '1';
}
function renderNotifications() {
  ...
  if(!list.length) { el.innerHTML = '<div class="empty-state">Илгээсэн мэдэгдэл олдсонгvй</div>'; return; }
  el.innerHTML = `<table class="data-table">
    <thead><tr><th>Огноо</th><th>Төрөл</th><th>Гарчиг</th><th>Хvлээн авагч</th><th>Тоо</th><th>Суваг</th></tr></thead>
    <tbody>${list.map(n => `
...
// Мөр дээр дарахад бодит агуулга, бvх хvлээн авагчийн жагсаалтыг харуулна
function openNotifDetail(id) {
  ...
  body.innerHTML = `
    ...
    <div style="font-weight:700;font-size:12.5px;margin:14px 0 8px;text-transform:uppercase;letter-spacing:.03em;color:var(--text-secondary)">Хvлээн авагчид (${recipients.length})</div>
    <div style="max-height:260px;overflow-y:auto">
      ${recipients.length ? recipients.map(r => `
        ...
        </div>`).join('') : '<div class="empty-state">Хvлээн авагчийн мэдээлэл алга</div>'}
    </div>`;
