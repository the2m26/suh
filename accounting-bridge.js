...
// ⚠️ 2026-07-23 нэмэв: Дээрхийн адил "ямар ч 1 бичилт байна уу" гэсэн ерөнхий
// асуултын оронд, "Энэ САРЫН reference-үүдийн ЖАГСААЛТ" гаргаж авах — үүнийг
// НЭХЭМЖЛЭЛ табанд мөр (хүн) БүРИЙГ тусад нь шалгахад ашиглана. Ингэснээр
// хэсэгчлэн (жиш нь 4/54) бүртгэгдсэн үед, аль хэдийн бүртгэгдсэн хүний
// "устгах" товч л нуугдаж, үлдсэн хүмүүсийнх нь харагдсаар байна.
async function _fetchAccruedReferencesForMonth(yearMonth) {
  const { data, error } = await sb.from('journal_entries').select('reference').ilike('reference', `accrual:%:${yearMonth}`);
  if (error) { console.error(error); return new Set(); }
  return new Set((data || []).map(r => r.reference));
}
...
// ------------------------------------------------------------
// ЖУРНАЛД НЭХЭМЖЛЭХ БүРТГЭХ (Нягтлан бодох бүртгэл → "Журналд нэхэмжлэх бүртгэх" tab).
...
// ============================================================
let invoiceSchedule = { accrueDay: 1, sendDay: 1, dueDay: 25, channels: ['inapp'] };
...
async function loadInvoiceSchedule() {
  ...
  if (data?.value) invoiceSchedule = { accrueDay: 1, sendDay: 1, dueDay: 25, channels: ['inapp'], ...data.value };
...
async function renderInvoiceScheduleSettings() {
  ...
  const ch = invoiceSchedule.channels || ['inapp'];
  document.getElementById('invsched-ch-email').checked = ch.includes('email');
  document.getElementById('invsched-ch-sms').checked = ch.includes('sms');
  document.getElementById('invsched-ch-inapp').checked = ch.includes('inapp');
}
...
async function saveInvoiceScheduleSettings() {
  ...
  const channels = [];
  if (document.getElementById('invsched-ch-email').checked) channels.push('email');
  if (document.getElementById('invsched-ch-sms').checked) channels.push('sms');
  if (document.getElementById('invsched-ch-inapp').checked) channels.push('inapp');
  if (!channels.length) { toast('Дор хаяж нэг суваг сонгоно уу', 'error'); return; }
  const value = { ...invoiceSchedule, accrueDay, sendDay, dueDay, channels };
...
async function recordMonthlyInvoice(auto) {
  ...
  const failedRows = [];
  // ⚠️ 2026-07-23 засав (2 давхар алдаа): (1) Үүний өмнө энэ давталтад
  // try/catch огт байгаагүй байсан тул, аль нэг оршин суугч/ААН-ий мэдээлэл
  // боловсруулах үед exception шидвэл БүХ ДАВТАЛТ тэр дор нь зогсож, үлдсэн
  // бүх хүлээн авагчийг боловсруулалгүйгээр орхидог байсан (54-өөс зөвхөн 4
  // бүртгэгдэж, 50 нь "боловсруулагдаагүй" хэвээрээ үлдсэн шалтгаан яг энэ).
  // (2) "Бүтэн сарын" idempotency шалгалт (accountingCheckAlreadyAccrued,
  // ЯМАР ч 1 бичилт байхад бүгдийг блоклодог байсан) орхигдсон 50-ыг дараа нь
  // сэргээх боломжгүй болгож байсан тул, шалгалтыг МӨР БүРЭЭР нь (тухайн
  // хүлээн авагчийн reference аль хэдийн байгаа эсэх) хийдэг болгов — үүгээр
  // үүнийг дахин ажиллуулахад аль хэдийн бүртгэгдсэн 4-ийг алгасаад, үлдсэн
  // 50-ыг л боловсруулна.
  for (const row of rows) {
    const reference = row.type === 'resident'
      ? `accrual:resident:${row.label}:${yearMonth}`
      : `accrual:business:${row.raw.id}:${yearMonth}`;
    try {
      const { data: existing, error: existErr } = await sb.from('journal_entries').select('id').eq('reference', reference).limit(1);
      ...
      if (row.type === 'resident') {
        ...
        res.success ? succeeded++ : (failed++, failedRows.push(row.label));
      } else {
      ...
      console.error('Нэхэмжлэл бүртгэхэд exception:', row.label, e);
      failed++; failedRows.push(row.label);
    }
  }
  if (failedRows.length) console.error('Нэхэмжлэл бүртгэгдээгүй хүлээн авагчид:', failedRows);
  // ⚠️ 2026-07-23 нэмэв: Товч давхардуулж дарахад ("бүгд аль хэдийн бүртгэгдсэн"
  // тохиолдол) гарч ирдэг тоастыг илүү ойлгомжтой болгов. Тоо (54 гэх мэт)
  // үргэлж `succeeded`/`skipped`-ээс шууд авдаг, хатуу кодлогдоогүй.
  let summary = `${succeeded} ширхэг нэхэмжлэл бүртгэгдлээ.`;
  ...
  // нэхэмжлэх бүртгэх" (энд) ба "Нэхэмжлэл илгээх" одоо 2 ТУСДАА, өөр өөр календарийн
  // өдөр дээр ажилладаг үйлдэл (НББ тохиргоо → Нэхэмжлэл таб үзнэ vv).
}

// ⚠️ 2026-07-27 нэмэв: Нэг сууц өмчлөгч/ААН-д зориулсан нэхэмжлэлийн бичвэрийг
// үүсгэдэг ЭХ СУРВАЛЖ функц — batch илгээлт (_sendInvoiceNotifications, доор)
// болон "Тоот/Сууц өмчлөгч/ААН дэлгэрэнгүй → Мэдэгдэл" товчоор дангаар нь
// бэлдэх (notifications.js, openNotifyFor) хоёулаа ЭНЭ НЭГ функцийг ашиглана —
// формат хоёр газар хэзээ ч зөрөхгүй.
async function buildInvoiceContentFor(kind, entity, yearMonth) {
  yearMonth = yearMonth || `${CUR_YEAR}-${String(CUR_MONTH).padStart(2, '0')}`;
  const [y, m] = yearMonth.split('-').map(Number);
  ...
  let amount;
  if (kind === 'resident') {
    amount = calcFee(entity);
    lines.push(`${monthLabel} сарын СӨХ-ийн төлбөр нэхэмжлэгдлээ:`);
    feeCatalog.filter(f => f.active && f.applies_to === 'resident').forEach(f => {
      const amt = Math.round(_feeQuantity(entity, 'resident', f.unit_type) * (+f.rate || 0));
      if (amt) lines.push(`- ${f.name}: ${fmtMoney(amt)}`);
    });
  } else {
    amount = computeBizFee(entity);
    lines.push(`${monthLabel} сарын түрээс/СӨХ-ийн төлбөр нэхэмжлэгдлээ:`);
    if (entity.type === 'owner') lines.push(`- Талбайн төлбөрөөс чөлөөлөгдсөн (Өмчлөгч)`);
    feeCatalog.filter(f => f.active && f.applies_to === 'business').forEach(f => {
      const amt = Math.round(_feeQuantity(entity, 'business', f.unit_type) * (+f.rate || 0));
      if (amt) lines.push(`- ${f.name}: ${fmtMoney(amt)}`);
    });
  }
  lines.push(`Нийт: ${fmtMoney(amount)}`, dueLine);
...
async function _sendInvoiceNotifications(rows, yearMonth) {
  ...
  const recipients = [];
  for (const row of rows) {
    // ⚠️ 2026-07-27 засав: Гараар давхар бичихийн оронд ЯГ НЭГ эх сурвалж
    // (buildInvoiceContentFor()) дуудаж, дангаар нь бэлдэх хувилбартай ЯГ ИЖИЛ
    // форматтай байгааг баталгаажуулав.
    const content = await buildInvoiceContentFor(row.type, row.raw, yearMonth);
  ...
  const notifRow = {
    ...
    category: 'invoice', channels: invoiceSchedule.channels || ['inapp'], recipients_snapshot: recipients,
  ...
  if ((invoiceSchedule.channels||['inapp']).includes('inapp')) triggerPushForRecipients(recipients, `${monthLabel} сарын нэхэмжлэх`);
}
...
// ⚠️ 2026-07-26 засав: computeResidentFee() function-ыг арилгав — ЯГ ЭНЭ
// давхардал (calcFee()-ээс тусдаа, Зогсоол/Агуулахыг зөв тооцдог хувилбар)
// нь calcFee()-тэй харилцан зөрсөн 3 томьёоны нэг байсан. Одоо доод талд
// шууд calcFee(r) дуудна — ЯГ НЭГ л эх сурвалж (fee_catalog).
