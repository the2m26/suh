...
async function recordMonthlyInvoice(auto) {
  ...
  if (!auto && !confirm(`${yearMonth} сарын хураамж/түрээсийг ${rows.length} хүлээн авагчид нэхэмжлэх vv?\n(Энэ үйлдлийг буцаах боломжгүй тул анхаарна уу.)`)) return;
...
// 0 гэж хуурамч харуулахгүй). "Хугацаандаа төлнө уу" гэдгийг НББ тохиргоо →
// Нэхэмжлэл табанд тохируулсан өдрөөр сольсон.
async function _sendInvoiceNotifications(rows, yearMonth) {
  ...
  const recipients = rows.map(row => {
    ...
    if (row.type === 'resident') {
      ...
      const util = feeSettings.utility || 0; // ⚠️ Тариф тохиргоонд бодит нэр нь "Нэмэлт зардал" ("Ашиглалтын зардал" биш — тэр нь Зарлагын ангиллын өөр ойлголт)
      ...
      const lines = [
        `${monthLabel} сарын СӨХ-ийн хураамж нэхэмжлэгдлээ:`,
        `- Тоотын төлбөр (${sqm}м²): ${fmtMoney(sqmFee)}`,
        `- Нэмэлт зардал: ${fmtMoney(util)}`,
      ];
      ...
      if (isTenant) lines.push(`- Нэмэлт үйлчилгээний төлбөр: ${fmtMoney(extraFee)}`);
      lines.push(`Нийт: ${fmtMoney(row.amount)}`, dueLine);
      content = lines.join('\n');
    }
  ...
  const notifRow = {
    type: 'invoice', title: `${monthLabel} сарын нэхэмжлэх`, content: `${monthLabel} сарын хураамж/түрээсийн нэхэмжлэх`,
