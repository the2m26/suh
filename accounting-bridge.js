...
async function _sendInvoiceNotifications(rows, yearMonth) {
  ...
  const recipients = rows.map(row => {
    let content;
    // ⚠️ 2026-07-26 засав: Мэдэгдлийн бичвэрт харуулах задаргааг ГАРААР
    // хуулбарлахын оронд ЯГ НЭГ эх сурвалжаас (fee_catalog) шууд уншиж
    // байгуулна — calcFee()/computeBizFee()-тэй ямар ч зөрчил vvсэхгvй.
    if (row.type === 'resident') {
      const r = row.raw;
      const rows2 = feeCatalog.filter(f => f.active && (f.applies_to==='resident' || f.applies_to==='both'));
      const lines = [`${monthLabel} сарын СӨХ-ийн төлбөр нэхэмжлэгдлээ:`];
      rows2.forEach(f => {
        const qty = _feeQuantity(r, 'resident', f.unit_type);
        const amt = Math.round(qty * (+f.rate||0));
      ...
      const b = row.raw;
      const rows2 = feeCatalog.filter(f => f.active && (f.applies_to==='business' || f.applies_to==='both'));
      const lines = [`${monthLabel} сарын түрээс/СӨХ-ийн төлбөр нэхэмжлэгдлээ:`];
      if (b.type === 'owner') lines.push(`- Талбайн төлбөрөөс чөлөөлөгдсөн (Өмчлөгч)`);
      rows2.forEach(f => {
        const qty = _feeQuantity(b, 'business', f.unit_type);
        const amt = Math.round(qty * (+f.rate||0));
        if (amt) lines.push(`- ${f.name}: ${fmtMoney(amt)}`);
      });
      lines.push(`Нийт: ${fmtMoney(row.amount)}`, dueLine);
