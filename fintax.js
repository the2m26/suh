...
// "СӨХ-д санал хүсэлт илгээх" tile-аас ирнэ)
// ============================================================
async function loadCallLogInbox() {
  ...
  if (!body) return;
  if (error) { body.innerHTML = `<tr><td colspan="5">Ачаалахад алдаа гарлаа: ${esc(error.message)}</td></tr>`; return; }
  if (!data || !data.length) { body.innerHTML = `<tr><td colspan="5" style="color:var(--text-muted)">Одоогоор ирсэн санал, хүсэлт алга</td></tr>`; return; }
  body.innerHTML = data.map(r => {
    ...
    return `
      <tr data-id="${r.id}">
        <td>${dtStr}</td>
        <td>${esc(String(r.apt))}</td>
        <td>${esc(r.sender_name || '')}</td>
        <td style="max-width:360px;white-space:pre-wrap">${esc(r.content || '')}</td>
        <td>
