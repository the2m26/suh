...
function renderCC2ThreadView(apt, resident) {
  ...
  const bubbles = _cc2ActiveMessages.map(m => {
    ...
    return `${dayDivider}
      <div style="display:flex;flex-direction:column;max-width:62%;align-self:${isOut ? 'flex-end' : 'flex-start'}">
        <div style="padding:10px 14px;border-radius:14px;font-size:13px;line-height:1.5;${isOut ? 'background:var(--accent);color:#fff;border-top-right-radius:4px' : 'background:var(--bg-card);border:1px solid var(--border);border-top-left-radius:4px'}">${esc(m.text || '')}</div>
  ...
  view.innerHTML = `
    ...
    <div style="border-top:1px solid var(--border);padding:14px 20px;background:var(--bg-surface);display:flex;gap:10px;align-items:flex-end">
      <textarea id="cc2-reply-text" placeholder="Хариу бичих..." style="flex:1;background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:10px 14px;color:var(--text);font-size:13px;font-family:inherit;resize:none;height:44px"></textarea>
