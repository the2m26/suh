...
async function renderSystemStartStatus() {
  ...
  const { data } = await sb.from('settings').select('*').eq('key', 'system_start').maybeSingle();
...
async function activateSystemStart() {
  ...
  try {
    ...
    await sb.from('settings').upsert({ key: 'system_start', value: { date: dateVal, force }, updated_at: new Date().toISOString() }, { onConflict: 'key' });
...
async function renderAppSettingsModulesList() {
  ...
  if (!el) return;
  const { data } = await sb.from('settings').select('*').eq('key', 'mobile_modules').maybeSingle();
