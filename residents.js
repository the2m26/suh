  ...
  body.innerHTML=list.map((r,idx)=>{
    ...
    return `<tr style="cursor:pointer" onclick="openResidentDetail(${r.id})">
      ...
      <td><div class="flex gap-8">
        ${canCopy('residents')?`<button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();copyResident(${r.id})" style="padding:4px;display:inline-flex;align-items:center;color:var(--text-muted)" title="Хуулбарлах"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>`:''}
        ...
        ${!canEditRes&&!canDelRes&&!canCopy('residents')?'<span style="color:var(--text-muted);font-size:11px">—</span>':''}
      </div></td>
    </tr>`;
  }).join('');
...
// Хуулбарлах — editResident()-ийн бүх талбарыг pre-fill хийгээд, editingResidentId-г
// null болгосноор saveResident() UPDATE биш шинэ INSERT хийнэ. Тоот (apt) давхцаж
// болохгүй өвөрмөц утга тул хоослож, хэрэглэгчээр шинээр сонгуулна.
function copyResident(id) {
  if(!canCopy('residents')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
  editResident(id);
  editingResidentId = null;
  document.getElementById('modal-resident-title').textContent = 'Сууц өмчлөгч хуулбарлах (Тоот сонгоно уу)';
