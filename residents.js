...
function openAddResident() {
  if(!canAdd('residents')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
...
async function saveResident() {
  ...
  // Засах горимд байхад тухайн бичлэг олдохгүй бол чимээгүй хуулбар үүсгэхийн
  // оронд тодорхой алдаа өгнө.
  if(editingResidentId && !residents.find(r=>r.id===editingResidentId)) {
    toast('Засварлах гэж буй сууц өмчлөгч олдсонгүй — хуудсаа дахин ачаалаад дахин оролдоно уу', 'error');
    return;
  }
  ...
  if(editingResidentId) {
    ...
    logActivity('edit', 'residents', data.dbId, `${aptId} — ${data.firstname} ${data.lastname}`);
  } else {
    residents.push({id:nextId++,...data});
    toast(`${aptId} тоот өмчлөгч нэмэгдлээ ✓`,'success');
    logActivity('add', 'residents', data.dbId, `${aptId} — ${data.firstname} ${data.lastname}`);
  }
...
async function deleteResident(id) {
  ...
  logActivity('delete', 'residents', dbId, r ? `${r.apt} — ${r.firstname} ${r.lastname}` : String(id));
}
...
// ⚠️ 2026-08-05 бүрэн шинэчлэв: сараар (бүхэл тоогоор) тооцдог байснаас
// өдрөөр нарийвчилсан тооцоолол руу шилжив. Асуудал: 7-р сарын төлбөр
// төлөгдөөгүй байхад 8-р сарын нэхэмжлэх аль хэдийн илгээгдсэн үед,
// сараар тооцоход "1 сар л төлөгдөөгүй" гэж гарч, "хугацаа хэтэрсэн"
// эрхэм рүү шилжихгүй байсан (2 сар болтол хүлээх шаардлагатай байсан) —
// бодит байдал дээр үнэхээр хугацаа хэтэрсэн байсан ч гэсэн.
// Одоо: (1) хамгийн эртний төлөгдөөгүй сарыг олно, (2) түүний нэхэмжлэх
// илгээгдсэн өдрийг (invoiceSchedule.sendDay-аар тооцоолсон) олно,
// (3) өнөөдрөөс тэр хүртэлх бодит хоногийн ялгааг буцаана.
function daysUnpaidForResident(r) {
  // ⚠️ Cosmo-ийн виртуал резидент мөр (isVirtual) үргэлж 0 буцаана.
  ...
  if (lastPay) {
    ...
    if (isNaN(od)) return 999 * 30; // өмнөх (999 сар) зан төлөвтэй ойролцоо хоногийн сентинел
    unpaidMonth = od.getMonth() + 1; unpaidYear = od.getFullYear();
  } else {
    return 999 * 30;
  }
...
function _aptDetailResidentHTML(res, buildingId, floor, label, sqm, paid, history) {
  ...
  // ⚠️ 2026-07-26 засав: Энэ мөрөнд гараар хуулбарлагдсан томьёо calcFee()-тэй
  // харилцан зөрж байсан (Зогсоол/Агуулах тооцдог ч, calcFee() тооцдоггүй байсан).
  // Одоо ЯГ НЭГ л эх сурвалж (fee_catalog) ашиглана.
  const totalFee = calcFee(res);
  ...
  const feeRows = (typeof feeCatalog !== 'undefined' ? feeCatalog : [])
    .filter(f => f.active && f.applies_to === 'resident')
    .map(f => ({ name: f.name, amt: Math.round(_feeQuantity(res, 'resident', f.unit_type) * (+f.rate || 0)) }))
    .filter(x => x.amt);
  const feeSection = `
    ...
    ${feeRows.map(x=>`<div class="summary-row"><span class="summary-key">${esc(x.name)}</span><span class="summary-val font-mono">${fmt(x.amt)}</span></div>`).join('')}
