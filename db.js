...
const ACTLOG_MODULE_LABELS = {
  ...
  fintax: 'Санхүү, татварын тайлан', notifications: 'Зар мэдэгдэл илгээх', news: 'Мэдээ, мэдээлэл', polls: 'Сонгууль, санал асуулга',
...
async function renderActivityLogPage() {
  ...
  const yearEl = document.getElementById('actlog-year-filter');
  if (yearEl && !yearEl.options.length) {
    const curYear = new Date().getFullYear();
    yearEl.innerHTML = '<option value="">Он</option>' +
      Array.from({length:4}, (_,i) => curYear-i).map(y => `<option value="${y}">${y}</option>`).join('');
  }
  ...
  // Он сонгоогүй бол хуучинчлан хамгийн сүүлийн 400 бичлэгийг л татна (бүх
  // хугацааны түүхийг нэг дор дуудахаас зайлсхийнэ).
  if (yearFilter) {
    const y = +yearFilter, m = monthFilter ? +monthFilter : null, d = dayFilter ? +dayFilter : null;
    let start, end;
    if (m && d) { start = new Date(y, m-1, d); end = new Date(y, m-1, d+1); }
    else if (m) { start = new Date(y, m-1, 1); end = new Date(y, m, 1); }
...
async function db_init() {
  ...
  try {
    ...
    await Promise.all([
      ...
      // ⚠️ 2026-08-05 нэмэв: Хүлээлттэй/Хугацаа хэтэрсэн/Эрсдэлтэй τөлβөрийн
      // төлөв тооцоолол одоо "Нэхэмжлэх илгээсэн өдρөөс хойш хэдэн хоног"
      // (invoiceSchedule.sendDay) дээр түшигладаг тул, ЗААВАЛ бүх апп нээгдэх
      // бүрд ачаалагдах ёстой — үгүй бол τүүхий анхны утга (1) хэвээр үлдэнэ.
      (typeof loadInvoiceSchedule === 'function' ? loadInvoiceSchedule() : Promise.resolve())
    ]);
    // Аж ахуйн нэгжийн сарын төлбөрийг (Талбай × Тариф) БҮХ өгөгдөл (fee_catalog
    // хамт) бүрэн ачаалагдсаны ДАРАА дахин тооцоолно — Promise.all зэрэг ажилладаг
    // тул businesses/каталог аль нь эрт дуусахаас үл хамааран үргэлж зөв тариф
  ...
  onExpTypeChange();
  updateSidebarCount();
  if (typeof _checkInvoiceScheduleAutoTriggers === 'function') _checkInvoiceScheduleAutoTriggers();
}
