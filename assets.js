...
function openAddAsset() {
  if(!canAdd('assets')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
  editingAssetId = null;
...
async function saveAsset() {
  ...
  if(editingAssetId) {
    ...
    logActivity('edit', 'assets', data.dbId, name);
  } else {
    assets.push({id:nextId++, ...data});
    toast(name+' нэмэгдлээ','success');
    logActivity('add', 'assets', data.dbId, name);
  }
...
async function deleteAsset(id) {
  ...
  logActivity('delete', 'assets', a?.dbId || id, a?.name || String(id));
}
