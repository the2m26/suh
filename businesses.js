...
// төлсөөр байх ёстой.
// ⚠️ 2026-07-26 засав: Хатуу кодлогдсон rentSettings.perSqm/garage/storageSqm/
// waste/extra-г тус тусад нь нэмдэг байсныг ХАСАВ — одоо ЯГ НЭГ эх сурвалжаас
// (fee_catalog, finance.js-ийн calcEntityFee()) тооцно. Өмчлөгч (type==='owner')
// зөвхөн ГАЗРЫН ТАЛБАЙН төлбөрөөс чөлөөлөгдөх зарчим calcEntityFee()/_feeQuantity()
// дотор хэвээр хадгалагдсан.
function computeBizFee(b) {
  return calcEntityFee(b, 'business');
}
...
function openBusinessDetail(id) {
  ...
  const feeRows = feeCatalog.filter(f => f.active && (f.applies_to==='business' || f.applies_to==='both'))
    .map(f => ({ name: f.name, unitType: f.unit_type, amt: Math.round(_feeQuantity(b, 'business', f.unit_type) * (+f.rate||0)) }));
  const totalFee = feeRows.reduce((s,x)=>s+x.amt, 0);
