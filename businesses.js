...
function computeBizFee(b) {
  ...
  return areaFee + garageFee + storageFee + (rentSettings.waste || 0) + (rentSettings.extra || 0);
}
function updateBizFeePreview() {
  ...
  el.textContent = type === 'owner'
    ? 'Өмчлөгч компанид сарын төлбөр ногдохгүй'
    : `≈ ${fmtMoney(computeBizFee({ area, type, parkings, storages }))}₮ / сар (тариф тохиргооны дагуу)`;
}
...
function openBusinessDetail(id) {
  ...
  const wasteFee = isTenant ? (rentSettings.waste||0) : 0;
  const extraFee = isTenant ? (rentSettings.extra||0) : 0;
  const totalFee = isTenant ? (areaFee+garFee+storFee+wasteFee+extraFee) : 0;
  ...
  const feeSection = isTenant ? `
    ...
    <div class="summary-row"><span class="summary-key">Түрээсийн талбайн төлбөр (${+b.area||0}м²)</span><span class="summary-val font-mono">${fmtMoney(areaFee)}</span></div>
    ...
    <div class="summary-row"><span class="summary-key">Нэмэлт үйлчилгээний төлбөр</span><span class="summary-val font-mono">${fmtMoney(extraFee)}</span></div>
    ...
    <div style="font-size:10px;font-weight:700;color:var(--text-muted);letter-spacing:.08em;text-transform:uppercase;margin:12px 0 6px">СӨХ-ийн төлбөр</div>
    <div class="summary-row"><span class="summary-key" style="color:var(--text-dim)">Өмчлөгч компанид сарын төлбөр ногдохгүй</span></div>`;
...
function openBizPayModal(b) {
  ...
  if (b.type === 'owner') {
    ...
    const area = +b.area || 0;
    const areaFee = area * (rentSettings.perSqm || 0);
    ...
    const extraFee = rentSettings.extra || 0;
    ...
    bd.innerHTML = `<div style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:8px">📋 ${esc(b.name)} — СӨХ-ийн төлбөрийн задаргаа</div>
      <div class="summary-row"><span class="summary-key">Түрээсийн талбайн төлбөр (${area}м²)</span><span class="summary-val font-mono">${fmtMoney(areaFee)}</span></div>
