...
function computeBizFee(b) {
  ...
  const extraFee = rentSettings.extra || 0; // ⚠️ 2026-07-20: Өмчлөгч ЗӨВХӨН Талбайн төлбөрөөс чөлөөлөгдөнө — Нэмэлт үйлчилгээг ч төлнө (глобал дүрэм)
  if (b.type === 'owner') return garageFee + storageFee + wasteFee + extraFee;
  const areaFee = (+b.area || 0) * (rentSettings.perSqm || 0);
  return areaFee + garageFee + storageFee + wasteFee + extraFee;
}
...
function openAddBusiness() {
  if(!canAdd('businesses')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
...
function openBusinessDetail(id) {
  ...
  const extraFee = rentSettings.extra||0; // ⚠️ Өмчлөгч ЗӨВХӨН Талбайн төлбөрөөс чөлөөлөгдөнө — үүнийг ч төлнө
  const totalFee = isTenant ? (areaFee+garFee+storFee+wasteFee+extraFee) : (garFee+storFee+wasteFee+extraFee);
  ...
  const feeSection = `
    ...
    <div class="summary-row"><span class="summary-key">Нэмэлт үйлчилгээ</span><span class="summary-val font-mono">${fmtMoney(extraFee)}</span></div>
...
async function saveBusiness() {
  ...
  if(editingBusinessId && !businesses.find(b=>b.id===editingBusinessId)) {
    toast('Засварлах гэж буй аж ахуйн нэгж олдсонгүй — хуудсаа дахин ачаалаад дахин оролдоно уу', 'error');
    return;
  }
  ...
  if(editingBusinessId) {
    ...
    logActivity('edit', 'businesses', data.dbId, name);
  } else {
    businesses.push({id:nextId++,...data});
    toast(name+' нэмэгдлээ','success');
    logActivity('add', 'businesses', data.dbId, name);
  }
...
async function deleteBusiness(id) {
  ...
  logActivity('delete', 'businesses', b?.dbId || id, b?.name || String(id));
}
// ============================================================
...
function openAddClientele() {
  if(!canAdd('clientele')) { toast('Танд энэ үйлдлийг хийх эрх байхгүй байна','error'); return; }
...
async function saveClientele() {
  ...
  // ⚠️ 2026-07-19: assets.js-ийн адил ноцтой алдаанаас сэргийлэх хамгаалалт
  if(editingClienteleId && !clientele.find(c=>c.id===editingClienteleId)) {
    toast('Засварлах гэж буй харилцагч олдсонгүй — хуудсаа дахин ачаалаад дахин оролдоно уу', 'error');
    return;
  }
  ...
  if(editingClienteleId) {
    ...
    toast('Мэдээлэл шинэчлэгдлээ','success');
    logActivity('edit', 'clientele', data.dbId, legalName);
  } else {
    clientele.push({id:nextId++, ...data});
    toast(legalName+' нэмэгдлээ','success');
    logActivity('add', 'clientele', data.dbId, legalName);
  }
...
async function deleteClientele(id) {
  if(!confirm('Устгах уу?')) return;
  const c = clientele.find(x=>x.id===id);
  try {
  ...
  updateSidebarCount();
  toast('Устгагдлаа','success');
  logActivity('delete', 'clientele', c?.dbId || id, c?.legalName || String(id));
}
