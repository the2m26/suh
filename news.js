...
function _renderNewsCard(p) {
  const dateStr = p.published_at ? new Date(p.published_at).toLocaleDateString('mn-MN') : '';
  ...
  <div class="card news-card" style="padding:18px;margin-bottom:16px">
    ...
    <div class="news-text-wrapper news-truncated">
      <p style="white-space:pre-wrap;margin:0;color:var(--text-dim);line-height:1.5">${esc(p.body)}</p>
    </div>
