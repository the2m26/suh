...
// ⚠️ 2026-07-26 нэмэв: Мэдээний "body" (rich HTML) талбарыг innerHTML-д
// ХАРУУЛАХ бүрдээ DOMPurify-аар дахин цэвэрлэнэ (хадгалах үед newseditor.js
// аль хэдийн цэвэрлэдэг ч, ХОЁР ДАВХАР хамгаалалт — хуучин, цэвэрлэгдээгүй
// хадгалагдсан бичлэг байсан ч энд аюулгүй болно). news.js эхэлж ачаалагддаг
// тул энэ функцийг ЭНД нэг цэгт л тодорхойлж, newseditor.js үүнийг дуудна
// (давхар тодорхойлохгүй — файл эвдрэлээс сэргийлэх; userapp-react нь тусдаа
// React bundle тул энэ global функцийг дуудахгүй, өөрийн sanitize логиктой).
const NEWS_SANITIZE_CONFIG = {
  ALLOWED_TAGS: ['p', 'div', 'br', 'b', 'strong', 'i', 'em', 'u', 'a', 'font', 'span', 'ul', 'ol', 'li'],
  ALLOWED_ATTR: ['href', 'target', 'color'],
  ALLOW_DATA_ATTR: false,
};
function sanitizeNewsBody(dirtyHtml) {
  if (typeof DOMPurify === 'undefined') {
    console.error('DOMPurify ачаалагдаагvй — мэдээний HTML цэвэрлэгдэхгvй тул хоосон харуулна');
    return '';
  }
...
// ⚠️ 2026-07-24 нэмэв: Мэдээний "Topic" ангилал — эдитор дээр сонгож,
// үзэх хуудсанд шүүлтүүр болгож ашиглана ("мини CMS" загвар). "value" нь
// dropdown-ы утга, "label" нь мэдээний картны огнооны мөрөнд "| Label" гэж
// бүтэн нэрээр нь харагдана (жиш: "УЗ шийдвэр" → "Удирдах зөвлөлийн шийдвэр").
const NEWS_TOPICS = [
...
function _newsTopicLabel(value) {
...
async function renderNewsPage() {
  ...
  _allNewsPosts = data || [];
  _renderNewsFilterBar();
  _applyNewsTopicFilter();
}

function _renderNewsFilterBar() {
  const wrap = document.getElementById('news-filter-bar');
  if (!wrap || wrap.dataset.built) return;
  wrap.dataset.built = '1';
  wrap.innerHTML = `<select id="news-topic-filter" onchange="_applyNewsTopicFilter()" style="width:auto;min-width:200px">
    <option value="">Бүгдийг харах</option>
    ${NEWS_TOPICS.map(t => `<option value="${t.value}">${esc(t.label)}</option>`).join('')}
  </select>`;
}

function _applyNewsTopicFilter() {
  const el = document.getElementById('news-list');
  if (!el) return;
  const filterVal = document.getElementById('news-topic-filter')?.value || '';
  const filtered = filterVal ? _allNewsPosts.filter(p => p.topic === filterVal) : _allNewsPosts;
  if (!filtered.length) { el.innerHTML = '<div class="empty-state">Одоогоор мэдээ алга</div>'; return; }
  el.innerHTML = filtered.map(p => _renderNewsCard(p)).join('');
  _initNewsInteractivity();
  _trackNewsViews(filtered);
}

// ⚠️ 2026-07-25 нэмэв: "үзсэн" тоог хэт олон удаа (дахин ачаалах бүр) нэмэгдэхээс
// сэргийлж, localStorage-д "үзсэн ID"-г хадгалаад, БРАУЗЕР БүРД зөвхөн НЭГ УДАА л
// үнэн зүйл нэмэгдэнэ (increment_news_view() RPC — Supabase талд atomic).
const NEWS_VIEWED_KEY = 'suh_news_viewed_ids';
function _getNewsViewedIds() {
  try { return new Set(JSON.parse(localStorage.getItem(NEWS_VIEWED_KEY) || '[]')); }
  catch (e) { return new Set(); }
}
function _saveNewsViewedIds(set) {
  localStorage.setItem(NEWS_VIEWED_KEY, JSON.stringify([...set]));
}
async function _trackNewsViews(posts) {
  const viewed = _getNewsViewedIds();
  for (const p of posts) {
    if (viewed.has(p.id)) continue;
    viewed.add(p.id);
    const { error } = await sb.rpc('increment_news_view', { p_id: p.id });
    if (error) { console.error('үзсэн тоолуур алдаа:', error); continue; }
    const span = document.getElementById('news-view-' + p.id);
    if (span) span.textContent = String((p.view_count || 0) + 1).padStart(3, '0');
  }
  _saveNewsViewedIds(viewed);
}

function _renderNewsCard(p) {
  const dateStr = p.published_at ? _fmtNewsCardDate(new Date(p.published_at)) : '';
  const topicLabel = _newsTopicLabel(p.topic);
  ...
  <div class="card news-card" style="padding:18px;margin-bottom:16px">
    ...
    <div style="font-size:11px;color:var(--text-muted);margin-bottom:4px">${esc(dateLine)} | Үзсэн: <span id="news-view-${p.id}">${String(p.view_count || 0).padStart(3, '0')}</span></div>
    <h3 style="font-size:16px;margin:0 0 10px;color:var(--text);text-transform:uppercase;line-height:1.3">${esc(p.title)}</h3>
    <div class="news-text-wrapper news-truncated news-body">${sanitizeNewsBody(p.body)}</div>
