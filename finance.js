...
// unit_type-ын дэлгэцэнд ЗӦВХӦН 3 цэвэр сонголт харагдана (Талбайгаар/Тоогоор/
// Тогтмол) — гэхдээ дотооддоо main_sqm/storage_sqm тусдаа хэвээр (тооцоолол
// өөрчлөгдөхгvй, зөвхөн УИ-д нэгэн адил "Талбайгаар" гэж харагдана).
// "locked" мөрvvд (Тоот/ААН/Зогсоол/Агуулах — Хаягжилт тохиргооны бодит тоот/
// зогсоол/агуулахтай шууд уяатай) — эдгээрийн Тооцооллын аргыг ХЭЗЭЭ Ч санамсаргvй
// сольж болохгvй тул dropdown-ыг disable, устгах товчийг нуана. Нэр/Хэмжээ хэвээр засварлана.
...
function renderFeeCatalogTable(tab) {
  ...
  tbody.innerHTML = rows.map(f => {
    ...
    return `<tr>
      <td style="font-weight:600">${f.locked?'<span title="Хаягжилт тохиргооны бодит тоот/зогсоол/агуулахтай шууд уяатай — нэр/хэмжээг л засна" style="margin-right:5px">🔒</span>':''}${esc(f.name)}</td>
...
function openFeeCatalogModal(id, defaultTab) {
  ...
  unitSel.value = (f && f.unit_type === 'storage_sqm') ? 'main_sqm' : (f ? f.unit_type : 'main_sqm');
  unitSel.disabled = !!(f && f.locked);
...
async function saveFeeCatalogRow() {
  ...
  const row = {
    name,
    // ⚠️ locked мөрийн Тооцооллын аргыг УИ дээр disable хийсэн ч, form submit
    // vед хуучин утгыг ХАТУУ хадгалж (санамсаргvй солигдохоос сэргийлж), disable
    // биш мөрийн хувьд л сонгосон утгыг авна.
    unit_type: (existing && existing.locked) ? existing.unit_type : document.getElementById('fee-catalog-unit-type').value,
