...
let feeSettings = {penalty: 2, fundAmount: 5000000, pendingDays: 25, overdueDays: 35, riskDays: 365};
...
function populateTariffFields() {
  const feeMap = {
    ...
    'fee-pending-days': feeSettings.pendingDays, 'fee-overdue-days': feeSettings.overdueDays, 'fee-risk-days': feeSettings.riskDays,
  };
  const rentMap = {
    'rent-penalty': rentSettings.penalty,
    'rent-pending-days': rentSettings.pendingDays, 'rent-overdue-days': rentSettings.overdueDays, 'rent-risk-days': rentSettings.riskDays,
  };
  const gateMap = {
...
let rentSettings = {penalty: 2, pendingDays: 25, overdueDays: 35, riskDays: 365};
...
async function saveRentSettings() {
  ...
  rentSettings.pendingDays = _numField('rent-pending-days', 25);
...
async function saveFeeSettings(){
  ...
  feeSettings.pendingDays=_numField('fee-pending-days', 25);
...
function _bizThresholds(b) {
  ...
  return {pending: rentSettings.pendingDays||25, overdue: rentSettings.overdueDays||35, risk: rentSettings.riskDays||365};
}
...
function _renderPayPending(body, paidAptIds, filter='') {
  ...
  const pendingThreshold = feeSettings.pendingDays || 25;
