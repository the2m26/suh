...
function _feeQuantity(entity, entityType, unitType) {
  ...
  if (unitType === 'main_count') {
    // Нэг сууц/ААН = 1 нэгж — "Тоогоор" сонголт Тоот/ААН мөрөнд vргэлж 1 (flat-тай тоон vр дvнгээрээ ижил, гэхдээ тусдаа нэртэй, нуугдмал холбоос биш).
    if (entityType === 'business' && entity.type === 'owner') return 0;
    return 1;
  }
...
const FEE_OBJECT_UNIT_OPTIONS = {
  ...
  custom: [ ['main_sqm','Талбайгаар (₮/м²/сар)'], ['parking_count','Тоогоор (₮/ш/сар) — Зогсоолын тоогоор'], ['flat','Тогтмол (₮/сар)'] ],
};
...
function calcRentPreview() {
  ...
  // ⚠️ 2026-07-27 засав: Талбай/Зогсоол/Агуулах оролтын талбарууд одоо ЗӨВХӨН
  // тэдгээрийг БОДИТООР ашигладаг идэвхтэй мөр байгаа vед л харагдана — идэвхгvй
  // (ашиглагдахгvй) хэмжигдэхvvнд харагдахгvй, нуугдмал тоогоор тооцохгvй.
  const needMain = rows.some(f => f.unit_type === 'main_sqm');
  const needParking = rows.some(f => f.unit_type === 'parking_sqm' || f.unit_type === 'parking_count');
  const needStorage = rows.some(f => f.unit_type === 'storage_sqm' || f.unit_type === 'storage_count');
  const sqmGroup = document.getElementById('rent-preview-sqm-group'); if (sqmGroup) sqmGroup.style.display = needMain ? '' : 'none';
  const gGroup = document.getElementById('rent-preview-garage-group'); if (gGroup) gGroup.style.display = needParking ? '' : 'none';
  const sGroup = document.getElementById('rent-preview-storage-group'); if (sGroup) sGroup.style.display = needStorage ? '' : 'none';
  const noteEl = document.getElementById('rent-preview-no-input-note'); if (noteEl) noteEl.style.display = (!needMain && !needParking && !needStorage) ? '' : 'none';
  ...
  const parkingRow = feeCatalog.find(f => f.object_type === 'parking' && f.applies_to === 'business');
  const storageRow = feeCatalog.find(f => f.object_type === 'storage' && f.applies_to === 'business');
  const gLabel = document.getElementById('rent-preview-garage-label');
  if (gLabel) gLabel.textContent = 'Зогсоол' + (parkingRow && parkingRow.unit_type === 'parking_sqm' ? ' (м²)' : ' (ширхэг)');
  const sLabel = document.getElementById('rent-preview-storage-label');
  if (sLabel) sLabel.textContent = 'Агуулах' + (storageRow && storageRow.unit_type === 'storage_count' ? ' (ширхэг)' : ' (м²)');

  let total = 0; const lines = [];
  rows.forEach(f => {
    let qty;
    if (f.unit_type==='flat') qty = 1;
    else if (f.unit_type==='main_count') qty = 1;
    else if (f.unit_type==='main_sqm') qty = sqm;
    else if (f.unit_type==='parking_sqm' || f.unit_type==='parking_count') qty = garages;
    else if (f.unit_type==='storage_sqm' || f.unit_type==='storage_count') qty = stSqm;
    else qty = 0;
...
function calcFeePreview() {
  ...
  // ⚠️ 2026-07-27 засав: Талбай/Зогсоол/Агуулах оролтын талбарууд одоо ЗӨВХӨН
  // тэдгээрийг БОДИТООР ашигладаг идэвхтэй мөр байгаа vед л харагдана.
  const needMain = rows.some(f => f.unit_type === 'main_sqm');
  const needParking = rows.some(f => f.unit_type === 'parking_sqm' || f.unit_type === 'parking_count');
  const needStorage = rows.some(f => f.unit_type === 'storage_sqm' || f.unit_type === 'storage_count');
  const sqmGroup = document.getElementById('preview-sqm-group'); if (sqmGroup) sqmGroup.style.display = needMain ? '' : 'none';
  const gGroup = document.getElementById('preview-garage-group'); if (gGroup) gGroup.style.display = needParking ? '' : 'none';
  const sGroup = document.getElementById('preview-storage-group'); if (sGroup) sGroup.style.display = needStorage ? '' : 'none';
  const noteEl = document.getElementById('preview-no-input-note'); if (noteEl) noteEl.style.display = (!needMain && !needParking && !needStorage) ? '' : 'none';

  const sqm=_numField('preview-sqm', 95);
  const garages=_numField('preview-garage', 0);
  const stSqm=_numField('preview-storage-sqm', 0);
  const parkingRow = feeCatalog.find(f => f.object_type === 'parking' && f.applies_to === 'resident');
  const storageRow = feeCatalog.find(f => f.object_type === 'storage' && f.applies_to === 'resident');
  const gLabel = document.getElementById('preview-garage-label');
  if (gLabel) gLabel.textContent = 'Зогсоол' + (parkingRow && parkingRow.unit_type === 'parking_sqm' ? ' (м²)' : ' (ширхэг)');
  const sLabel = document.getElementById('preview-storage-label');
  if (sLabel) sLabel.textContent = 'Агуулах' + (storageRow && storageRow.unit_type === 'storage_count' ? ' (ширхэг)' : ' (м²)');
