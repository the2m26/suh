...
function calcEntityFee(entity, entityType) {
  const rows = feeCatalog.filter(f => f.active && (f.applies_to === entityType || f.applies_to === 'both'));
...
const FEE_UNIT_LABELS = {
  main_sqm: { badge:'₮/м²/сар', tag:'tag-accent', rateLabel:'Хэмжээ (₮/м²/сар)' },
  storage_sqm: { badge:'Агуулах ₮/м²/сар', tag:'tag-accent', rateLabel:'Хэмжээ (₮/м²/сар)' },
  parking_count: { badge:'₮/ширхэг/сар', tag:'tag-warning', rateLabel:'Хэмжээ (₮/ширхэг/сар)' },
  flat: { badge:'Тогтмол ₮/сар', tag:'tag-success', rateLabel:'Хэмжээ (₮/сар)' },
};
const FEE_APPLIES_LABELS = {
  resident: { text:'Сууц', tag:'tag-accent' }, business: { text:'ААН', tag:'tag-warning' }, both: { text:'Хоёул', tag:'tag-success' },
};
function renderFeeCatalogTable(tab) {
  ...
  if (!tbody) return;
  const rows = feeCatalog.filter(f => f.applies_to === tab || f.applies_to === 'both')
    .sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));
  if (countEl) countEl.textContent = `${rows.length} төлбөрийн мөр`;
  if (!rows.length) { tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state">Төлбөрийн мөр алга</div></td></tr>'; return; }
  tbody.innerHTML = rows.map(f => {
    const u = FEE_UNIT_LABELS[f.unit_type] || FEE_UNIT_LABELS.flat;
    const a = FEE_APPLIES_LABELS[f.applies_to] || FEE_APPLIES_LABELS.both;
    return `<tr>
      <td style="font-weight:600">${esc(f.name)}</td>
      <td><span class="tag ${u.tag}">${u.badge}</span></td>
      <td class="font-mono">${fmtMoney(f.rate)}₮</td>
      <td><span class="tag ${a.tag}">${a.text}</span></td>
      ...
      <td><div class="flex gap-8">
        ...
        <button class="btn btn-ghost btn-sm" style="padding:4px;color:var(--danger)" title="Устгах" onclick="deleteFeeCatalogRow(${f.id})"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button>
      </div></td>
    </tr>`;
  }).join('');
}
...
function openFeeCatalogModal(id, defaultTab) {
  ...
  document.getElementById('fee-catalog-name').value = f ? f.name : '';
  document.getElementById('fee-catalog-unit-type').value = f ? f.unit_type : 'main_sqm';
  document.getElementById('fee-catalog-rate').value = f ? f.rate : '';
  document.getElementById('fee-catalog-applies-to').value = f ? f.applies_to : (defaultTab || 'resident');
...
async function saveFeeCatalogRow() {
  ...
  const name = document.getElementById('fee-catalog-name').value.trim();
  ...
  const row = {
    ...
    applies_to: document.getElementById('fee-catalog-applies-to').value,
...
function calcRentPreview() {
  ...
  const rows = feeCatalog.filter(f => f.active && (f.applies_to==='business' || f.applies_to==='both'));
...
function calcFeePreview() {
  ...
  const rows = feeCatalog.filter(f => f.active && (f.applies_to==='resident' || f.applies_to==='both'));
  let total = 0; const lines = [];
  rows.forEach(f => {
    let qty;
    if (f.unit_type==='flat') qty = 1;
...
function onPayAptChange() {
  ...
  if(String(val).startsWith('biz:')) {
    ...
    const feeRows = feeCatalog.filter(f => f.active && (f.applies_to==='business' || f.applies_to==='both'))
  ...
  const feeRowsR = feeCatalog.filter(f => f.active && (f.applies_to==='resident' || f.applies_to==='both'))
