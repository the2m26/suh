...
function calcRentPreview() {
  ...
  el.innerHTML = `
    ...
    <div class="summary-row"><span class="summary-key">Нэмэлт үйлчилгээний төлбөр</span><span class="summary-val">${fmtMoney(extra)}</span></div>
...
function onPayAptChange() {
  ...
  if(String(val).startsWith('biz:')) {
    ...
    bd.innerHTML=`<div style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:8px">📋 ${esc(b.name)} — СӨХ-ийн төлбөрийн задаргаа</div>
      ...
      ${isTenant?`<div class="summary-row"><span class="summary-key">Нэмэлт үйлчилгээний төлбөр</span><span class="summary-val font-mono">${fmt(extraFee)}</span></div>`:''}
...
async function savePayment() {
  ...
  if(!isQuickMode && String(rawVal).startsWith('biz:')) {
    ...
    const data={
      ...
      description: b.name+' — '+month+'-р сарын СӨХ-ийн төлбөр', subcat:'Сарын хураамж',
    };
