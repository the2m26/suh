...
async function db_loadSettings() {
  ...
  data.forEach(s=>{
    ...
    if(s.key==='gate_tariff') Object.assign(gateTariff, s.value);
  });
}
...
let feeSettings = {penalty: 2, fundAmount: 5000000, overdueDays: 35, riskDays: 365};
...
// болгож болохгүй — Edge Function-тэй холбоо тасрана).
let gateTariff = { parking_rate_minutes: 60, parking_rate_amount: 0, temp_stop_free_minutes: 15, guest_free_minutes: 60 };
...
function switchTariffTab(name, el) {
  ['tariff-fund','tariff-fees','tariff-rent','tariff-gate'].forEach(id=>{const e=document.getElementById(id);if(e)e.style.display='none';});
...
function populateTariffFields() {
  const feeMap = {
    ...
    'fee-overdue-days': feeSettings.overdueDays, 'fee-risk-days': feeSettings.riskDays,
  };
  const rentMap = {
    'rent-penalty': rentSettings.penalty,
    'rent-overdue-days': rentSettings.overdueDays, 'rent-risk-days': rentSettings.riskDays,
  };
  const gateMap = {
    'gate-parking-rate-minutes': gateTariff.parking_rate_minutes, 'gate-parking-rate-amount': gateTariff.parking_rate_amount,
    'gate-temp-stop-free-minutes': gateTariff.temp_stop_free_minutes, 'gate-guest-free-minutes': gateTariff.guest_free_minutes,
  };
  Object.entries({...feeMap, ...rentMap, ...gateMap}).forEach(([id,val])=>{
    const el = document.getElementById(id);
...
// Хаалтны тариф (ANPR камер, gate-webhook Edge Function) — 2026-07-30 нэмэв
async function saveGateTariffSettings(){
  gateTariff.parking_rate_minutes = _numField('gate-parking-rate-minutes', 60);
  gateTariff.parking_rate_amount = _numField('gate-parking-rate-amount', 0);
  gateTariff.temp_stop_free_minutes = _numField('gate-temp-stop-free-minutes', 15);
  gateTariff.guest_free_minutes = _numField('gate-guest-free-minutes', 60);
  const ok = await db_saveSettings('gate_tariff', gateTariff);
  if(!ok) { toast('Хадгалахад алдаа гарлаа — консол шалгана уу','error'); return; }
  toast('Хаалтны тариф хадгалагдлаа ✓','success');
}
// ============================================================
...
const FEE_UNIT_LABELS = {
  ...
  main_count: { badge:'Тоогоор (₮/ш/сар)', tag:'tag-warning', rateLabel:'Хэмжээ (₮/ширхэг/сар)' },
...
const FEE_OBJECT_UNIT_OPTIONS = {
  ...
  // ⚠️ 2026-07-27 засав: Шинэ (custom) мөрд ерөнхий "Тоогоор" гэсэн сонголт
  // ӨГӨӨГүй — учир нь ямар ч бодит "тоо" өгөгдөл (Зогсоол/Агуулахаас бусад)
  // системд байдаггүй тул, ямар нэг бүдэг "Тоогоор" сонгоход аль нэг рүү
  // НУУГДМАЛААР холбогдох эрсдэлтэй байсан (яг иймээс алдаа гарсан). Одоо
  // ЯГ ЮУГ тоолохыг НЭРЭНД НЬ ШУУД, тодорхой бичсэн 2 тусдаа сонголт өгнө.
...
let rentSettings = {penalty: 2, overdueDays: 35, riskDays: 365};
function calcRentPreview() {
  const rows = feeCatalog.filter(f => f.active && f.applies_to==='business')
    .sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));
  // 2026-07-27 бүрэн дахин зохион байгуулав: мөр бүр (объектын "төрлөөр"
  // бүлэглэхгүйгээр) ӨӨРИЙН тусдаа, бие даасан оролтын нүдтэй — өөр ямар ч
  // мөртэй хуваалцахгүй, нуугдмал холбоос үүсгэхгүй. Зөвхөн Талбайгаар/
  // Тоогоор аргатай идэвхтэй мөрүүдэд л нүд гарна (Тогтмол/main_count-д
  // нүд шаардлагагүй, автоматаар 1×хэмжээгээр тооцогдоно).
  const needsInput = rows.filter(f => f.unit_type !== 'flat' && f.unit_type !== 'main_count');
  const container = document.getElementById('rent-preview-inputs-dynamic');
  if (container) {
    const key = needsInput.map(f => f.id + ':' + f.unit_type).join(',');
    if (container.dataset.rowsKey !== key) {
      container.dataset.rowsKey = key;
      if (!needsInput.length) {
        container.innerHTML = '<div style="font-size:11.5px;color:var(--text-muted);margin-bottom:10px">Идэвхтэй бүх төлбөр тогтмол дүнтэй тул оролт шаардахгүй.</div>';
      } else {
        container.innerHTML = needsInput.map(f => {
          const isSqm = (f.unit_type === 'main_sqm' || f.unit_type === 'storage_sqm' || f.unit_type === 'parking_sqm');
          return `<div class="form-group"><label>${esc(f.name)} (${isSqm ? 'м²' : 'ширхэг'})</label><input type="number" id="rent-preview-qty-${f.id}" value="0" min="0" oninput="calcRentPreview()"></div>`;
        }).join('');
      }
    }
  }

  let total = 0; const lines2 = [];
  rows.forEach(f => {
    let qty;
    if (f.unit_type==='flat' || f.unit_type==='main_count') qty = 1;
    else qty = +(document.getElementById('rent-preview-qty-'+f.id)?.value) || 0;
    const amt = qty * (+f.rate||0);
    total += amt;
    lines2.push(`<div class="summary-row"><span class="summary-key">${esc(f.name)}</span><span class="summary-val">${fmtMoney(amt)}</span></div>`);
  });
  const el = document.getElementById('rent-preview-result'); if(!el) return;
  el.innerHTML = lines2.join('') + `
...
async function saveRentSettings() {
  ...
  rentSettings.overdueDays = _numField('rent-overdue-days', 35);
  rentSettings.riskDays = _numField('rent-risk-days', 365);
...
function calcFeePreview() {
  const rows = feeCatalog.filter(f => f.active && f.applies_to==='resident')
    .sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));
  // 2026-07-27 бүрэн дахин зохион байгуулав: мөр бүр ЭЗНИЙ тусдаа, бие даасан
  // оролтын нүдтэй — өөр ямар ч мөртэй хуваалцахгүй, нуугдмал холбоос
  // үүсгэхгүй (calcRentPreview()-той ЯГ ИЖИЛ загвар).
  const needsInput = rows.filter(f => f.unit_type !== 'flat' && f.unit_type !== 'main_count');
  const container = document.getElementById('preview-inputs-dynamic');
  if (container) {
    const key = needsInput.map(f => f.id + ':' + f.unit_type).join(',');
    if (container.dataset.rowsKey !== key) {
      container.dataset.rowsKey = key;
      if (!needsInput.length) {
        ...
        container.innerHTML = needsInput.map(f => {
          const isSqm = (f.unit_type === 'main_sqm' || f.unit_type === 'storage_sqm' || f.unit_type === 'parking_sqm');
          return `<div class="form-group"><label>${esc(f.name)} (${isSqm ? 'м²' : 'ширхэг'})</label><input type="number" id="preview-qty-${f.id}" value="0" min="0" oninput="calcFeePreview()"></div>`;
        }).join('');
      }
    }
  }
  ...
  rows.forEach(f => {
    let qty;
    if (f.unit_type==='flat' || f.unit_type==='main_count') qty = 1;
    else qty = +(document.getElementById('preview-qty-'+f.id)?.value) || 0;
    const amt = qty * (+f.rate||0);
    total += amt;
    lines2.push(`<div class="summary-row"><span class="summary-key">${esc(f.name)}</span><span class="summary-val">${fmt(amt)}</span></div>`);
  });
...
async function saveFeeSettings(){
  ...
  feeSettings.overdueDays=_numField('fee-overdue-days', 35);
  feeSettings.riskDays=_numField('fee-risk-days', 365);
...
function _payRow(r, bgColor, textColor, statusText, showBtn) {
  ...
  return `<tr style="cursor:pointer" onclick="openResidentDetail(${r.id})">
    ...
    <td class="dt-text" style="color:${textColor}">${daysUnpaidForResident(r)}+ хоног</td>
...
// ⚠️ 2026-08-05 бүрэн шинэчлэв: daysUnpaidForResident()-той яг ижил зарчим
// (сараас өдрөөр нарийвчлав) — доор тайлбарыг residents.js-ээс үзнэ үү.
function daysUnpaidForBusiness(b) {
  ...
  if (lastPay) {
    unpaidMonth = lastPay.month + 1; unpaidYear = lastPay.year;
    if (unpaidMonth > 12) { unpaidMonth = 1; unpaidYear += 1; }
  } else if (b.start) {
    const sd = new Date(b.start);
    if (isNaN(sd)) return 999 * 30;
    unpaidMonth = sd.getMonth() + 1; unpaidYear = sd.getFullYear();
  } else {
    return 999 * 30;
  }
  const sendDay = (typeof invoiceSchedule !== 'undefined' && invoiceSchedule.sendDay) || 1;
  const invoiceSentDate = new Date(unpaidYear, unpaidMonth - 1, sendDay);
...
function _bizThresholds(b) {
  ...
  return {overdue: rentSettings.overdueDays||35, risk: rentSettings.riskDays||365};
}
function _payRowBiz(b, bgColor, textColor, statusText, showBtn) {
  return `<tr style="cursor:pointer" onclick="openBusinessDetail(${b.id})">
    ...
    <td class="dt-text" style="color:${textColor}">${daysUnpaidForBusiness(b)}+ хоног</td>
...
// ⚠️ 2026-08-06 бүрэн шинэчлэв: "Хүлээлттэйд тооцох хугацаа" тусдаа талбарыг
// БҮРЭН арилгав — хэрэглэгчийн санал: 2 бодит хил (Хугацаа хэтэрсэн, Эрсдэлтэй)
// л хангалттай, Хүлээлттэй бол "Хугацаа хэтэрсэн хилээс бага" гэдгээр АВТОМАТААР
// тооцогдоно. Ингэснээр 2 талбар зөрчилдөж, зарим хоногийн утга ЯМАР ч табад
// ороогүй "алга болох" эрсдэл (өмнө нь илэрсэн) бүрмөсөн арилна.
function _renderPayPending(body, paidAptIds, filter='') {
  const overdueThreshold = feeSettings.overdueDays || 35;
  const list = residents.filter(r=>{
    ...
    const mu = daysUnpaidForResident(r);
    return mu >= 1 && mu < overdueThreshold;
  });
  ...
  const bizList = businesses.filter(b=>{
    ...
    const mu = daysUnpaidForBusiness(b);
    return mu >= 1 && mu < th.overdue;
  });
...
function _renderPayOverdue(body, paidAptIds, filter='') {
  const overdueThreshold = feeSettings.overdueDays || 35;
  const riskThreshold = feeSettings.riskDays || 365;
  const list = residents.filter(r=>{
    if(!r || paidAptIds.map(String).includes(String(r.apt))) return false;
    if(!_residentMatchesFilter(r, filter)) return false;
    const mu = daysUnpaidForResident(r);
...
function _renderPayRisk(body, filter='') {
  const riskThreshold = feeSettings.riskDays || 365;
  const riskResidents = residents.filter(r=>r && daysUnpaidForResident(r) >= riskThreshold && _residentMatchesFilter(r, filter));
  const riskBiz = businesses.filter(b=>{
    if(!b || !_bizMatchesFilter(b, filter)) return false;
    return daysUnpaidForBusiness(b) >= _bizThresholds(b).risk;
  });
  const residentRows = riskResidents.map(r=>{
    ...
    return `<tr style="cursor:pointer" onclick="openResidentDetail(${r.id})">
      ...
      <td class="dt-text" style="color:var(--danger)">${daysUnpaidForResident(r)}+ хоног</td>
      <td class="dt-muted">${_lastPaymentDateStrForResident(r)}</td>
      <td class="dt-muted">—</td>
  ...
  const bizRows = riskBiz.map(b=>{
    return `<tr style="cursor:pointer" onclick="openBusinessDetail(${b.id})">
      ...
      <td class="dt-text" style="color:var(--danger)">${daysUnpaidForBusiness(b)}+ хоног</td>
      <td class="dt-muted">${_lastPaymentDateStrForBusiness(b)}</td>
      <td class="dt-muted">—</td>
      <td style="color:var(--danger);font-size:12px;font-weight:600;white-space:nowrap">Эрсдэлтэй</td>
...
function switchPayTab(tab,el){
  ...
  el.classList.add('active');
  // ⚠️ 2026-08-05 нэмэв: "САР" багана Төлөгдсөн табад бодит τөлβөρийн сарыг,
  // харин Хүлээлттэй/Хугацаа хэтэрсэн/Эрсдэлтэй табад ХЭДЭН ХОНОГ хэтэрснийг
  // харуулдаг тул (өдρийн нарийвчлалтай тооцоолол руу шилжсэний дараа),
  // толгойн бичгийг табаас хамааруулан динамикаар сольж өгнө.
  const th = document.getElementById('pay-th-month-days');
  if (th) th.textContent = tab === 'completed' ? 'САР' : 'ХОЦРОЛТ';
