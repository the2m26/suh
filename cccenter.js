...
function renderCCThreadList() {
  ...
  container.innerHTML = list.map(t => {
    ...
    //   Muted  → avatar-ийн БУЛАН дээр жижиг mute SVG badge (detail view-тэй ижил хэв маяг)
    // ⚠️ 2026-08-04 засав (2): Muted индикаторыг бүтэн avatar-ийг бүрхдэг
    // тунгалаг давхаргаас, DETAIL VIEW-тэй ИЖИЛ жижиг булангийн badge болгож
    // сольсон — overlay арга нь inset:1px үед ч бүдэг pixel-хэмжээний холбогдол
    // (anti-aliasing) улаан/энгийн хүрээг бүдгэрүүлдэг асуудал үүсгэдэг байсан;
    // badge нь avatar-ийн ГАДНА буланд байрлах тул хүрээ/дэвсгэрт огт хүрдэггүй.
    const avatarBorder = st.urgent ? 'border:1px solid var(--danger)' : 'border:1px solid var(--border-light)';
    const muteIcon = st.muted ? `<div title="Muted" style="position:absolute;bottom:-2px;right:-2px;width:15px;height:15px;border-radius:50%;background:var(--text-muted);border:2px solid var(--bg-surface);display:flex;align-items:center;justify-content:center">
      <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 6 9H2v6h4l5 4V5z"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>
    </div>` : '';
    return `
...
function renderCCThreadView(apt, resident) {
  ...
  const avatarBorder = st.urgent ? 'border:1px solid var(--danger)' : 'border:1px solid var(--border-light)';
  const muteIcon = st.muted ? `<div style="position:absolute;bottom:-2px;right:-2px;width:16px;height:16px;border-radius:50%;background:var(--text-muted);border:2px solid var(--bg-surface);display:flex;align-items:center;justify-content:center">
    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 6 9H2v6h4l5 4V5z"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>
  </div>` : '';
