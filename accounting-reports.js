...
function exportSummaryRowsToXlsx(containerId, filename) {
  ...
  try {
    ...
    const btn = window.event?.target?.closest('[data-perm-module]');
    if (btn) logActivity('export', btn.dataset.permModule, null, filename.replace(/\.xlsx$/, ''));
  } catch(e) {
...
// ============================================================
// ⚠️ 2026-07-22 нэмэв: exportTableToXlsx('acct-ledger-data-table',...) шууд
// дуудахад, ХЭРЭВ данс сонгоогүй (эсвэл сонгосон данс hөдөлгөөнгүй) бол
// hүснэгэл DOM-д огт үүсэхгүй тул "Хүснэгт олдсонгүй" гэсэн будлиантай алдаа
// гардаг байсан. Үүнийг илүү ойлгомжтой мессежээр солив.
function exportLedgerTable() {
  const sel = document.getElementById('acct-ledger-account-select');
  if (!sel || !sel.value) { toast('Эхлээд данс сонгоно уу', 'error'); return; }
  const table = document.getElementById('acct-ledger-data-table');
  if (!table) { toast('Сонгосон дансанд экспортлох hөдөлгөөн алга', 'error'); return; }
  exportTableToXlsx('acct-ledger-data-table', 'Дансны_дэвтэр.xlsx');
}
...
async function renderInvoiceTab() {
  ...
  const sendBtn = document.getElementById('record-invoice-btn');

  let rows;
  if (isCurrentMonth) {
    // ⚠️ 2026-07-23 засав: Үүний өмнө "ямар ч 1 хүн бүртгэгдсэн бол БүГД
    // харагдахаа болино" гэсэн дүрэм байсан (editable = !alreadySent).
    // Одоо ЯГ ТУС БүРИЙН reference-ийг шалгаж, хүн бүрт нь тусад нь
    // "аль хэдийн бүртгэгдсэн vv" гэдгийг тэмдэглэнэ.
    rows = buildLiveInvoicePreviewRows();
    const accruedRefs = await _fetchAccruedReferencesForMonth(yearMonth);
    rows.forEach(r => {
      const reference = r.type === 'resident'
        ? `accrual:resident:${r.label}:${yearMonth}`
        : `accrual:business:${r.raw.id}:${yearMonth}`;
      r.alreadyAccrued = accruedRefs.has(reference);
    });
    const anyPending = rows.some(r => !r.alreadyAccrued);
    if (sendBtn) {
      sendBtn.style.display = canAccrue() ? '' : 'none';
      // ⚠️ 2026-07-23 засав: "Бүгд бүртгэгдсэн үед товч бүрэн алга болдог"
      // байсныг хэрэглэгч "систем эвдэрсэн юм шиг санагдана, 1460 гаруй
      // хүлээн авагчийг гүйлгэж харах цаг үгүй" гэж тайлбарлаад, товч
      // ХАРАГДАЖ БАЙХ ёстой гэж хүссэн — зүгээр л бүгд бүртгэгдсэн үед
      // ИДЭВХГүй (disabled) болгож, тодорхой tooltip үзүүлнэ. Хараахан
      // үлдсэн хүн байвал идэвхтэй хэвээрээ (аль хэдийн бүртгэгдсэн хүнийг
      // recordMonthlyInvoice() өөрөө reference-ээр шалгаж алгасдаг тул
      // зөвхөн үлдсэнийг л шинээр бүртгэнэ).
      sendBtn.disabled = !anyPending;
      sendBtn.title = anyPending
        ? 'Бүртгэсэн нэхэмжлэхийг цуцлах боломжгүйг анхаарна уу'
        : 'Бүх нэхэмжлэх Журналд бүртгэгдсэн байна';
    }
  } else {
    rows = await buildHistoricalInvoiceRows(yearMonth);
    rows.forEach(r => { r.alreadyAccrued = true; });
    if (sendBtn) {
  ...
  renderInvoiceRows(filtered, isCurrentMonth);
}

function renderInvoiceRows(rows, isCurrentMonth) {
  ...
  body.innerHTML = rows.map(r => {
    const canExclude = isCurrentMonth && !r.alreadyAccrued;
    return `
...
function switchNbbSettingsTab(name, el) {
  ...
  document.getElementById('nbb-settings-invoice-schedule').style.display = name === 'invoice-schedule' ? 'block' : 'none';
  ...
  if (name === 'invoice-schedule') renderInvoiceScheduleSettings();
}
...
async function renderTaxTab() {
  ...
  el.innerHTML = `
    <div class="flex-between mb-16" style="align-items:flex-start">
      ...
      <div class="flex gap-8">
        <button class="btn btn-outline btn-sm" data-perm-module="accounting" data-perm-action="print" onclick="printCurrentPage()" title="Хэвлэх">Хэвлэх</button>
        <button class="btn btn-outline btn-sm" data-perm-module="accounting" data-perm-action="export" onclick="exportSummaryRowsToXlsx('acct-tax','Татвар.xlsx')" title="Excel экспорт">Экспорт</button>
      </div>
    </div>
