...
async function openEditPoll(pollId) {
  ...
  const {data: questions, error: qLoadErr} = await sb.from('poll_questions').select('*').eq('poll_id', pollId).order('order_num');
  sbErr(qLoadErr, 'Асуулт ачаалах', {silent:true});
  ...
  if(poll.poll_type === 'issue' || poll.poll_type === 'rating') {
    ...
    for(let i=0; i<(questions||[]).length; i++) {
      const q = questions[i];
      const col = i===0 ? 'uz' : 'hz';
      const {data: options, error: optLoadErr} = await sb.from('poll_options').select('*').eq('question_id', q.id).order('order_num');
      sbErr(optLoadErr, 'Сонголт ачаалах', {silent:true});
...
async function savePoll() {
  if(!(editingPollId ? canWrite('polls') : canAdd('polls'))) { toast('Танд энэ vйлдлийг хийх эрх байхгvй байна', 'error'); return; }
  ...
  if(pollType === 'issue' || pollType === 'rating') {
    for(let i=0; i<currentPollQuestions.length; i++) {
      ...
      if(pollType === 'rating') {
        const {error: optErr} = await sb.from('poll_options').insert(
          [1,2,3,4,5].map(n=>({question_id: question.id, option_text: String(n), order_num: n-1}))
        );
        sbErr(optErr, 'Сонголт хадгалах');
      } else {
        const {error: optErr} = await sb.from('poll_options').insert([
    ...
    for(const cfg of configs) {
      ...
      const {error: candErr} = await sb.from('poll_options').insert(candidateOptions);
      sbErr(candErr, 'Нэр дэвшигч хадгалах');
    }
  }
...
async function openPollDetail(pollId) {
  ...
  const {data: questions, error: qErr} = await sb.from('poll_questions').select('*').eq('poll_id', pollId).order('order_num');
  sbErr(qErr, 'Асуулт ачаалах', {silent:true});
  ...
  for(const q of (questions||[])) {
    const {data: options, error: optErr} = await sb.from('poll_options').select('*').eq('question_id', q.id).order('order_num');
    sbErr(optErr, 'Сонголт ачаалах', {silent:true});
    const myResidentId = getMyResidentId();
    const {data: myVotes, error: voteErr} = await sb.from('votes').select('option_id').eq('question_id', q.id).eq('resident_id', myResidentId || -1);
    sbErr(voteErr, 'Миний санал ачаалах', {silent:true});
    ...
    if(poll.poll_type === 'rating' && (poll.show_results_live || poll.status === 'closed' || canManage)) {
      ...
      // ⚠️ 2026-07-22 нэмэв: Үнэлгээ (rating) төрлийн асуултын үр дүнг card
      // хэлбэрээр л харуулдаг тул (доорх <table>-той адилгүй), экспортын
      // функц (exportPollDetailToXlsx) ямар ч hүснэгэл олохгүй "олдсонгүй"
      // алдаа өгдөг байсан. Үзэгдэхгүй (display:none) hүснэгэл нэмж, ижил
      // экспортын механизмаар дамжуулав.
      html += `<table style="display:none">
        <thead><tr><th>Асуулт</th><th>Дундаж үнэлгээ</th><th>Үнэлсэн хүний тоо</th></tr></thead>
        <tbody><tr><td>${esc(q.question_text)}</td><td>${avg}</td><td>${totalCount}</td></tr></tbody>
      </table>`;
      continue;
    }
...
async function deleteDraftPoll(pollId) {
  if(!canDelete('polls')) { toast('Танд энэ vйлдлийг хийх эрх байхгvй байна', 'error'); return; }
  const poll = pollsList.find(p=>p.id===pollId);
...
function exportPollDetailToXlsx() {
  ...
  try {
    ...
    logActivity('export', 'polls', null, title);
  } catch(e) {
