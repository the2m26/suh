...
// ⚠️ 2026-07-20 нэмэв: Зогсоол/Агуулах эзэмшигчийг дарахад ӨМНӨ НЬ шууд Засах
// цонх (editResident/editBusiness) нээгддэг байсныг, бусад модулиудтай (Сууц
// өмчлөгч/ААН/Ажилтан/Хөрөнгө) нэгдсэн байдлаар "Мэдээлэл харах" дэлгэрэнгүй
// цонх нээгддэг болгов — тэр цонхны дотор аль хэдийн "Засах" товч бий.
function openSpotOwnerDetail(ownerType, ownerId) {
  if(ownerType==='resident') openResidentDetail(ownerId);
  else if(ownerType==='business') openBusinessDetail(ownerId);
}
...
function renderParkingList() {
  ...
  body.innerHTML = rows.map(row=>`<tr style="cursor:${row.owner?'pointer':'default'}" ${row.owner?`onclick="openSpotOwnerDetail('${row.owner.type}',${row.owner.entity.id})"`:''}>
